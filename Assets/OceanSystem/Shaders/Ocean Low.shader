    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct Varyings
            {
                ...
                half viewDepth     : TEXCOORD1;
                half4 positionNDC  : TEXCOORD2;
            ...
            Varyings OceanMainVert(Attributes input)
            {
                ...
                half3 viewVector = output.positionWS - _WorldSpaceCameraPos;
                half viewDist = length(viewVector);
                half viewDistXzSquared = dot(viewVector.xz, viewVector.xz);

                half warpDistance = Ocean_LengthScales.x * 0.5;
                ...
                half4 weights = LodWeights(viewDist, _CascadesFadeDist);
                output.positionWS += SampleDisplacement(output.worldUV, weights, 1);

                half3 positionOS = TransformWorldToObject(output.positionWS);
            ...
            half4 OceanMainFrag(Varyings input, half facing : VFACE) : SV_Target
            {
                float3 viewDir = _WorldSpaceCameraPos - input.positionWS;
                float viewDist = length(viewDir);
                viewDir = viewDir / viewDist;

                half4 lodWeights = LodWeights(viewDist, _CascadesFadeDist);

                half4 shoreWeights = 1;// ShoreModulation(i.shore.x);
                half4x4 derivatives = SampleDerivatives(input.worldUV, lodWeights * shoreWeights);
                half3 normal = NormalFromDerivatives(derivatives, 1);


                return half4(half3(0,0,1) + normal, 1);
                ...
                //half roughnessMap = SAMPLE_TEXTURE2D(_DistantRoughnessMap, sampler_DistantRoughnessMap,
                ...
                ////half submergence = SAMPLE_TEXTURE2D(Ocean_CameraSubmergenceTexture, 

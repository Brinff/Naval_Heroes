...
namespace Game.Pointer.Authoring
{
    ...
    public class PointerHandlerBaker : Baker<PointerHandlerAuthoring>
    {
        ...
        public static void Bake(IBaker baker, Entity entity, PointerHandlerEvent events)
        {
            ...
            if (events.HasFlag(PointerHandlerEvent.Enter))
            {
                baker.AddComponent<PointerEnterEvent>(entity);
                baker.SetComponentEnabled<PointerEnterEvent>(entity, false);
            }

            if (events.HasFlag(PointerHandlerEvent.Exit))
            {
                baker.AddComponent<PointerExitEvent>(entity);
                baker.SetComponentEnabled<PointerExitEvent>(entity, false);
            }
        }
    }
}

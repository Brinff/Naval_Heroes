...
namespace Code.IAP
{
    public class IAPSpecialOfferProcessor : MonoBehaviour, IIAPProcessor
    {
        ...
        [SerializeField]
        private float m_OldPriceMultiplier = 1;
        ...
        public void OnInitialize(Product product, IAPShopService shopService)
        {
            ...
            m_IsPurchased = new PlayerPrefsProperty<bool>(PlayerPrefsProperty.ToKey(nameof(IAPSpecialOfferProcessor), m_ProductID, nameof(m_IsPurchased)))
                .Build();
            m_StartDateTime = new PlayerPrefsProperty<long>(PlayerPrefsProperty.ToKey(nameof(IAPSpecialOfferProcessor), m_ProductID, nameof(m_StartDateTime)))

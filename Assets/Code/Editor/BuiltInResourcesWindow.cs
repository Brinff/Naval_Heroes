...
public class BuiltInResourcesWindow : EditorWindow
{
        ...
        if (Drawings == null)
        {
            ...
            else if (_showingIcons)
            {
                if (_objects == null)
                {
                    ...
                    _objects = _objects.Where(x => !string.IsNullOrEmpty(AssetDatabase.GetAssetPath(x)) && AssetDatabase.GetAssetPath(x).Contains("Library")).Where(x => !(x is Cubemap)).ToList();
                ...
                foreach (UnityEngine.Object oo in _objects)
                {
                    ...
                    draw.Draw = () =>
                    {
                        if (GUILayout.Button(texture.name, GUILayout.Width(width)))
                        {
                            ...
                            genericMenu.AddItem(new GUIContent("Get Code"), false, () =>
                            {
                                Debug.Log(AssetDatabase.GetAssetPath(texture));
                                CopyText("EditorGUIUtility.FindTexture( \"" + texture.name + "\" )");
                            });
                            genericMenu.AddItem(new GUIContent("Save"), false, () => { SaveTexture(texture); });
    ...
    {
        //string path = EditorUtility.SaveFilePanel("Save Texture", "Assets", texture.name, "png");
        //if (!string.IsNullOrEmpty(path))
        //{
        //    var temp = RenderTexture.GetTemporary(texture.width, texture.height);
        //    Graphics.Blit(texture, temp);
        //    var newTexture = temp.Bake(); 
